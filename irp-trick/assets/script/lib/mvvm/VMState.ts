    ...
    private setNodeState(node: cc.Node, checkState?: boolean) {
        ...
        switch (n) {
            ...
            case a.COMPONENT_CUSTOM:
                let comp = null;
                this.valueComponentName === 'cc.Node' ? comp = node : comp = node.getComponent(this.valueComponentName);
                ...
                if (this.valueComponentProperty in comp) {
                    ...
                    comp[this.valueComponentProperty] = check ? Number(this.valueComponentActionValue) : Number(this.valueComponentDefaultValue);
                }
                break;
